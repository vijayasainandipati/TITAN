"""
TITAN Ablation Model B — CNN-GRU on Engineered Statistical Features
Variant B: 1D-CNN temporal feature extractor + GRU operating on
windowed statistical cycle features (mean, std, p2p, slew), but NO physics residuals.
"""
...
class CNNGRUFeaturesBaseline(nn.Module):
    def __init__(self,
                 input_dim: int = 28, # Engineered cycle statistical features
                 hidden_dim: int = 64,
    ...
    def forward(self, x_seq: torch.Tensor) -> Dict[str, torch.Tensor]:
        # x_seq: (batch_size, seq_len, input_dim) -> transpose for Conv1d: (batch, input_dim, seq_len)
